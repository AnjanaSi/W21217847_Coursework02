import React, { useEffect, useState } from 'react'
import { deleteGenre, getAllGenres } from '../../utils/ApiFunctions';
import { Link } from 'react-router-dom';
import Paginator from '../common/Paginator';
import { FaEdit, FaPlus, FaTrashAlt } from "react-icons/fa";
import { Col, Row } from 'react-bootstrap';

const ExistingGenres = () => {
  const [genres, setGenres] = useState([]);
  const [currentPage, setCurrentPage] = useState(1);
  const [genresPerPage] = useState(8);
  const [isLoading, setIsLoading] = useState(false);
  const [successMessage, setSuccessMessage] = useState("");
  const [errorMessage, setErrorMessage] = useState("");

  useEffect(() => {
    fetchGenres();
  }, []);

  const fetchGenres = async () => {
    setIsLoading(true);
    try {
      const result = await getAllGenres();
      setGenres(result);
      setIsLoading(false);
    } catch (error) {
      setErrorMessage(error.message);
      setIsLoading(false);
    }
  };


  const handleDelete = async (genreId) => {
    try {
      const result = await deleteGenre(genreId);
      if (result === "") {
        setSuccessMessage(`Genre with ID ${genreId} was deleted.`);
        fetchGenres();
      } else {
        console.error(`Error deleting genre : ${result.message}`);
      }
    } catch (error) {
      setErrorMessage(error.message);
    }
    setTimeout(() => {
      setSuccessMessage("");
      setErrorMessage("");
    }, 3000);
  };

  const calculateTotalPages = (genres, genresPerPage) => {
    const totalGenres =genres.length;
    return Math.ceil(totalGenres / genresPerPage);
  };

  const handlePaginationClick = (pageNumber) => {
    setCurrentPage(pageNumber);
  };

  const indexOfLastGenre = currentPage * genresPerPage;
  const indexOfFirstGenre = indexOfLastGenre - genresPerPage;
  const currentGenres = genres.slice(indexOfFirstGenre, indexOfLastGenre);

  return (
    <>
      <div className="container col-md-8 col-lg-6">
        {successMessage && (
          <p className="alert alert-success mt-5">{successMessage}</p>
        )}

        {errorMessage && (
          <p className="alert alert-danger mt-5">{errorMessage}</p>
        )}
      </div>

      {isLoading ? (
        <p>Loading existing genres</p>
      ) : (
        <>
          <section className="mt-5 mb-5 container">
            <div className="d-flex justify-content-between mb-3 mt-5">
              <h2>Existing Genres</h2>
            </div>

            <Row>
              <Col md={6} className="mb-2 md-mb-0">
              </Col>

              <Col md={6} className="d-flex justify-content-end">
                <Link to={"/add-genre"}>
                  <FaPlus /> Add Genre
                </Link>
              </Col>
            </Row>

            <table className="table table-bordered table-hover">
              <thead>
                <tr className="text-center">
                  <th>ID</th>
                  <th>Name</th>
                  <th>Description</th>
                  <th>Actions</th>
                </tr>
              </thead>

              <tbody>
                {currentGenres.map((genre) => (
                  <tr key={genre.id} className="text-center">
                    <td>{genre.id}</td>
                    <td>{genre.name}</td>
                    <td>{genre.description}</td>
                    <td className="gap-2">
                      <Link to={`/edit-genre/${genre.id}`} className="gap-2">
                        <span className="btn btn-warning btn-sm ml-5">
                          <FaEdit />
                        </span>
                      </Link>
                      <button
                        className="btn btn-danger btn-sm ml-5"
                        onClick={() => handleDelete(genre.id)}
                      >
                        <FaTrashAlt />
                      </button>
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
            <Paginator
              currentPage={currentPage}
              totalPages={calculateTotalPages(
                genres,
                genresPerPage
              )}
              onPageChange={handlePaginationClick}
            />
          </section>
        </>
      )}
    </>
  );
}

export default ExistingGenres